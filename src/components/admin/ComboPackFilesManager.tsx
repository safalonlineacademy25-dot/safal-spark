...
interface ComboPackFilesManagerProps {
...
const ComboPackFilesManager = ({ 
  productId, 
  isNewProduct = false,
  onFilesChange,
  pendingFiles = []
}: ComboPackFilesManagerProps) => {
  ...
  // For new products, we show pending files instead of querying
  const displayFiles = isNewProduct ? pendingFiles : files.map(f => ({
    id: f.id,
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium">Combo Pack Files</label>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          ...
          >
            ...
            Add Files to Combo Pack
          </Button>
        </div>
      )}

      <p className="text-xs text-muted-foreground">
        Each file will be sent as a separate email to the customer. Files are sent in order.
      </p>
    </div>
  );
};

export default ComboPackFilesManager;
