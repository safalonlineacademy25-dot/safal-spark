...
async function sendWaSimpleTemplate(
  ...
  const url = `https://app.wasimple.in/api/v1/sendTemplateMessage?phoneId=${encodeURIComponent(phoneId)}&apiKey=${encodeURIComponent(apiKey)}&whatsappNumber=${encodeURIComponent(to)}`;
